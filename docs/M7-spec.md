...
| ~~Import of officers/birthdays from CSV or a paste~~ **— OVERTURNED 2026-07-26 by PLAN §11-M12/M13** | Was: "Not in PLAN §11-M7; the wizard is the entry path." The owner's post-v0.1.0 review found wizard data entry redundant (a member's name is typed up to three times per issue), so PLAN gained M12 (lodge address book: app-level roster, CSV/XLSX import, XLSX export) and M13 (roster-driven widgets: birthdays generate, officers/committees get pickers). **Do not re-defer this.** The wizard remains the entry path when the roster is empty. |
| ~~Per-widget icons beyond an `IconKey` string~~ **— OVERTURNED 2026-07-27 by PLAN §11-M16** | Was: "The App maps keys to glyphs; the icon set itself is M9 chrome work. Icons are never the only label." M9 shipped without the glyph set and so did M10–M15, leaving `IWidgetDefinition.IconKey` implemented by all six widgets and drawn by nothing. M16 ships `TrestleBoard.App/Theme/Icons.axaml` (24 closed-fill geometries on a 0–24 grid) and `Theme/ActionIcons.cs`, whose widget map is required by test to be exactly the six values `IconKey` returns. **Do not re-defer this.** The half of the row that was right stands and is now enforced rather than asserted: the App still maps keys to glyphs — `EditorAction` gained no icon field — and `IconTests.AnIconIsNeverTheOnlyContent` fails any button that shows a picture and no words. |
